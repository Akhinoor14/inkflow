    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: f ? 'column' : 'row', gap: 2, flexShrink: 0 }}>
          ...
          <div style={{ position: 'relative' }} ref={shapeBtnRef}>
            <ToolBtn icon={<Square size={15} />} label="Shapes" active={activeTool === 'shape'}
              onClick={() => {
                setActiveTool('shape');
                setShapeAnchorRect(shapeBtnRef.current?.getBoundingClientRect() ?? null);
                setShowShapeOpts(v => !v);
              }} />
            {showShapeOpts && <ShapeSelector onClose={() => setShowShapeOpts(false)} floatingMode={f} anchorRect={shapeAnchorRect} />}
          </div>
          <ToolBtn icon={<Sparkles size={15} />}
            label={handwritingSettings.shapeAutoConvert ? 'Auto-shape: ON (drawn strokes snap to shapes)' : 'Auto-shape: OFF (strokes stay as handwriting)'}
            active={handwritingSettings.shapeAutoConvert}
            onClick={() => setHandwritingSettings({ ...handwritingSettings, shapeAutoConvert: !handwritingSettings.shapeAutoConvert })} />
