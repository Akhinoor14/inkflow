...
interface CloseProp { onClose: () => void; floatingMode?: boolean; }

export function PenOptions({ onClose, floatingMode }: CloseProp) {
  ...
  return (
    <div ref={ref}
      className="absolute z-50 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-3 w-56 animate-fade-in"
      style={floatingMode ? { top: 0, left: '110%' } : { top: '110%', left: 0 }}>
...
export function ShapeSelector({ onClose, floatingMode, anchorRect }: CloseProp & { anchorRect?: DOMRect | null }) {
  const ref = useRef<HTMLDivElement>(null);
  ...
  // Fix: the toolbar container has overflow:auto (so it can scroll on small
  // screens), which clips any absolutely-positioned popup inside it —
  // that's why the shape picker never appeared. Render it through a portal
  // into document.body with fixed positioning anchored to the button instead.
  ...
  const menu = (
    <div ref={ref}
      className="z-50 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-2 animate-fade-in"
      style={style}>
