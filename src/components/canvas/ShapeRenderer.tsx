...
export function ShapeRenderer({ element, isSelected, transform, pageId }: Props) {
  ...
  const shape = (() => {
    switch (shapeType) {
      ...
      case 'star': {
        const cx = mcx, cy = mcy;
        const rOuterX = width / 2, rOuterY = height / 2;
        const rInnerX = rOuterX * 0.382, rInnerY = rOuterY * 0.382;
        const pts: string[] = [];
        for (let i = 0; i < 10; i++) {
        ...
        return <polygon points={pts.join(' ')} transform={rotAttr} strokeLinejoin="round" {...common} />;
      }
      default: return null;
    }
  })();
